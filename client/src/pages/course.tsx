...
export default function CoursePage() {
  ...
  const handleCopyRssFeed = async () => {
    if (!course?.rssFeedUuid) return;
    const feedUrl = `${window.location.origin}/feed/${course.rssFeedUuid}.xml`;
    try {
      await navigator.clipboard.writeText(feedUrl);
      toast({ title: "RSS feed URL copied to clipboard" });
    } catch {
      toast({ title: "Failed to copy RSS feed URL", variant: "destructive" });
    }
  };
